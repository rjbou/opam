...
let get_repo_files rt name dir =
  ...
  | OpamRepositoryRoot.Dir repo_root ->
    ...
    let dir = OpamFilename.Op.(dir / OpamRepositoryPathName.files_d) in
  ...
  | OpamRepositoryRoot.Tar tar ->
    ...
    let xfiles_dir =
      OpamFilename.Unix.Dir.of_string dir / OpamRepositoryPathName.files_d
    in
...
let read_package_opam_tar ~repo_name ~repo_root package_dir filename content extrafiles =
  ...
  | Some opam ->
    (try
       let nv =
         OpamPackage.of_string
           (OpamFilename.Unix.Base.to_string (OpamFilename.Unix.Dir.basename package_dir))
       in
       Some (nv, opam)
     with Failure _ ->
       log "ERR: directory name not a valid package: ignored %s"

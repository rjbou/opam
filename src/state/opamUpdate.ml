...
let fetch_dev_package url srcdir ?(working_dir=false) ?subpath nv =
  ...
  log "updating %a%a" (slog OpamUrl.to_string) remote_url
    (slog (OpamStd.Option.to_string OpamFilename.SubPath.to_pretty_string)) subpath;
...
let pinned_package st ?version ?(working_dir=false) name =
  ...
  | Some (opam, Some urlf) ->
    ...
    let save_overlay opam =
      let version = OpamFile.OPAM.version_opt opam +! version in
      OpamPinned.save_overlay st name ~version urlf opam
    in

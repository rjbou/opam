...
let from_2_0_to_v2_1 _ ~on_the_fly conf =
  ...
  let info_jobs_changed ~prev_jobs =
    if on_the_fly then
      OpamConsole.note
        "The 'jobs' option was reset for this launch, its new value will vary \
         according to the current number of cores on your machine."
    else
      OpamConsole.note
...
(* To add an upgrade layer
   * [from_x_to_y] updates only global config file. If repo or switch file need
     to be updated, it needs to be externalised in its own function
     [from_x_to_y_{repo,switch}] and guarded by [on_the_fly] condition, and a
     state returned as second part of the pair [`Repo | `Switch] option
   * [from_x_to_y] is added in [as_necessary] list of upgrades
   * [from_x_to_y_{repo,switch}] is added to [repo_switch_hard_upgrade] list of
     upgrades
*)

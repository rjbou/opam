...
let as_necessary requested_lock global_lock root config =
  ...
  let config =
    ...
    let local, global =
      List.partition OpamSwitch.is_external missing_switches
    in
    let tostr sws =
    ...
    let sentences = function
      ...
      | _ -> "es", "they no longer exist"
    in
    (if not on_the_fly && global <> [] then
       let switches, existence = sentences global in
       OpamConsole.warning "Removing global switch%s %s as %s"
         switches (tostr global) existence);
    (if not on_the_fly && local <> [] then
       let switches, existence = sentences local in
       OpamConsole.note "Removing local switch%s %s as %s"
         switches (tostr local) existence);
    config
  in

...
let pull_tree label ?cache_dir ?(cache_urls=[]) ?working_dir ?subpath
    local_dirname  =
  pulls_tree_t ?cache_dir ~cache_urls ?working_dir ?subpath
  [label, local_dirname]

let pulls_tree ?cache_dir ?(cache_urls=[]) dirnames checksums remote_urls =
  pulls_tree_t ?cache_dir ~cache_urls dirnames checksums remote_urls

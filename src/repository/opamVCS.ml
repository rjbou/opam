...
module type VCS = sig
  ...
  val fetch: ?cache_dir:dirname -> ?subpath:subpath -> dirname -> url -> unit OpamProcess.job
    ...
    let filter_subpath files =
      ...
      | Some sp ->
        OpamStd.List.filter_map
          (fun f ->
             if OpamStd.String.remove_prefix ~prefix:(OpamFilename.SubPath.to_string sp ^ Filename.dir_sep) f

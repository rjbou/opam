...
let get_diff repo1 repo2 =
  ...
  (if OpamFilename.Dir.equal
     ...
     log "diff: %a/{%a,%a}"
       (slog (fun t ->
            OpamFilename.Dir.to_string  (OpamFilename.dirname_dir t))) repo1
       (slog (fun t ->
            OpamFilename.Base.to_string (OpamFilename.basename_dir t))) repo1
       (slog (fun t ->
            OpamFilename.Base.to_string (OpamFilename.basename_dir t))) repo2
   else
     log "diff: %a vs %a"
       (slog OpamFilename.Dir.to_string) repo1
       (slog OpamFilename.Dir.to_string) repo2);
  let get_contents =

  ...
  let make_options file prefix script name mandir libdir stubsdir topdir docdir
    ...
    let file =
      ...
      | None ->
        ...
        match
          List.filter (fun f -> OpamFilename.check_suffix f OpamPathName.install_suffix)
            candidates
        with
        | [f] -> f
        | [] ->
    ...
    let pkgname = match name with
      ...
      | None
        when OpamFilename.check_suffix (OpamFile.filename file) OpamPathName.install_suffix ->

...
let install_compiler
  ...
  let base_comp = OpamSwitchState.compute_invariant_packages { t with installed = t.installed ++ to_install_pkgs } in
(*   OpamPackage.packages_of_names to_install_pkgs comp_roots in *)
  let has_comp_flag =
    let is_comp nv =

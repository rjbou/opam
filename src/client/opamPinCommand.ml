    ...
    (OpamStd.Option.to_string ~none:(bold "locally")
       (fun u -> Printf.sprintf "to %s%s"
           ...
           (OpamStd.Option.to_string (fun sp ->
                " " ^ OpamFilename.SubPath.to_pretty_string sp)
...
and source_pin
  ...
  log "pin %a to %a %a%a"
    ...
    (slog (OpamStd.Option.to_string OpamFilename.SubPath.to_pretty_string))
    subpath;
...
let scan ~normalise ~recurse ?subpath url =
  let open OpamStd.Option.Op in
  let pins_of_dir dir =
    OpamPinned.files_in_source_w_target ~locked:OpamStateConfig.(!r.locked <> None)
  ...
  if normalise then
    ...
    List.map (fun (name, version, url, sb) ->
        [ OpamPackage.Name.to_string name;
          ...
          OpamUrl.to_string url;
          (sb >>| OpamFilename.SubPath.to_normalised_string) +! "-" ]) pins

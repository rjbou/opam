...
let fatal_error ?(exit_code=1) =
  OpamConsole.error_and_exit (List.find (fun (_,i) -> i = exit_code) OpamStd.Sys.exit_codes |> fst)
...
let get_installed_packages packages =
  ...
  | "homebrew" ->
    ...
    |> OpamStd.String.Set.inter packages
  | "suse" | "opensuse" ->
    let lines =
  ...
  | "debian" ->
    ...
    in
    let lines =
    run_query_command "dpkg-query" (dpkg_args (OpamStd.String.Set.elements packages))
    in
    let installed =
      List.fold_left
        (fun acc l -> match string_split ' ' l with
           | [pkg;_;_;"installed"] -> OpamStd.String.Set.add pkg acc
           | _ -> acc)
        OpamStd.String.Set.empty lines in
    if OpamStd.String.Set.cardinal installed
       = OpamStd.String.Set.cardinal packages then
      installed
    else
    (* If package are missing look for virtual package. *)
    let missing = OpamStd.String.Set.inter installed packages in
    let resolve_virtual name =
      ...
      List.fold_left
        (fun acc l -> match string_split ' ' l with
           | pkg :: _ ->  OpamStd.String.Set.add pkg acc
           | [] -> acc)
        OpamStd.String.Set.empty lines
    in
    let virtual_map =
      OpamStd.String.Set.fold (fun vpkg acc ->
          OpamStd.String.Set.fold (fun pkg acc ->
              let old = try StringMap.find pkg acc with Not_found -> OpamStd.String.Set.empty in
              StringMap.add pkg (OpamStd.String.Set.add vpkg old) acc)
             (resolve_virtual vpkg)acc)
         missing StringMap.empty
    in
    ...
    List.fold_left
      (fun acc l -> match string_split ' ' l with
         | [pkg;_;_;"installed"] ->
           (match StringMap.find_opt pkg virtual_map with
            | Some p -> OpamStd.String.Set.union p acc
            | None -> acc)
         | _ -> acc)
      installed lines
  ...
  | "bsd" ->
    (match distribution with
     ...
     | "openbsd" ->
       ...
       |> OpamStd.String.Set.of_list
       |> OpamStd.String.Set.inter packages
     | _ -> OpamStd.String.Set.empty)
  | "macports" -> OpamStd.String.Set.empty
  | _ -> OpamStd.String.Set.empty
  ...
  let cmd, args =
    ...
    | "openbsd", _ ->
      ...
      if Unix.getuid () <> 0 then (
        ...
        if interactive && not (OpamConsole.confirm ~default:true "Allow ?") then
...
let install ~su ~interactive packages =
  if OpamStd.String.Set.is_empty packages then ()
  else
  let cmds =
    install_packages_commands ~interactive (OpamStd.String.Set.elements packages)
  in
  let ok =
    List.fold_left (fun ok cmd ->
        ok &&
        (sudo_run_command ~su ~interactive cmd))
      true cmds
  in
  if ok then OpamConsole.errmsg "# OS packages installation successful\n%!"

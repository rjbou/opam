    ...
    let gits, gfw_message =
      if gits = [] then
        ...
        | _ ->
          ...
          let test_for_installation ((gits, gfw_message) as acc) (reg, key) =
            let process root =
              ...
              && is_git_for_windows git then
                ...
                in
                gits, OpamStd.Option.default_map message gfw_message
              else
                acc
            in
            let key = Filename.concat key "GitForWindows" in
            OpamStubs.readRegistry reg key "InstallPath" OpamStubsTypes.REG_SZ
          ...
          List.fold_left test_for_installation (gits, None) installations
      else
        gits, None
    in

...
let opams_of_dir ?recurse ?subpath ?locked dir =
  opams_of_dir_t (OpamPinned.files_in_source ?recurse ?subpath ?locked)
    (fun (n, f, _) -> n, f)
    (fun n (_, f, s)-> n, f, s)
    dir

let opams_of_dir_w_target ?recurse ?subpath ?locked
    ?(same_kind=fun _ -> OpamClientConfig.(!r.pin_kind_auto)) url dir =
  opams_of_dir_t
    (OpamPinned.files_in_source_w_target
       ?recurse ?subpath ?locked ~same_kind url)
    (fun (n, f, _, _) -> n, f)

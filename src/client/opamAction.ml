...
let download_same_source_packages st url nvs =
  ...
  then Done None else
  let nvs =
    (* we pinned remove package that already has their source *)
    List.filter (fun nv ->
  ...
  OpamProcess.Job.catch (fun e ->
      ...
      Done (Some na))
  @@ fun () ->
  OpamUpdate.download_same_packages_source st url nvs @@| function
...
let download_package st nv =
  download_same_source_packages st

...
let patch ~allow_unclean ?patch_filename ~dir diffs =
  if diffs = [] then () else
    ...
    in
    let remove_dir file = OpamSystem.rmdir_cleanup (Filename.dirname file) in
    let apply diff =
      try
        (match diff.Patch.operation with
         ...
         | Patch.Git_ext (_, _, Patch.Rename_only (src, dst)) ->
           ...
           let check_and_write_dst longest_path content =
             if exists_dir longest_path then
    ...
    let rec loop diffs =
      let remaining =
        let rec aux remaining = function
          | diff::rem ->
            (match apply diff with
             | Ok () -> aux remaining rem
             | Error rej -> aux (rej::remaining) rem)
          | [] ->  remaining
        in
        aux [] diffs
      in

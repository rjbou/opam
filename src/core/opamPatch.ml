...
let patch_t (type a) (module C : PATCH_CONF with type root = a)
 ...
 if diffs = [] then () else
  ...
  let patch_info_path =
    OpamStd.Option.default
      (Printf.sprintf "in %s %s" C.root_label (C.root_to_string to_patch))
      patch_filename
  in
  let to_patch = C.end_slash to_patch in
  let get_path file =
    let fail () =
 ...
 let patch file content diff patching =
  ...
  C.open_ to_patch (fun patching ->
      let patched : C.target = List.fold_left apply patching diffs in
      C.save patched)
...
let patch (type a) (module C : PATCH_CONF with type root = a)
  ...
  try
    ...
    | `Patch_file p ->
      let diffs =
        let translate =
          if C.translate_patch then Some (C.root_to_string to_patch)
          else None
        in

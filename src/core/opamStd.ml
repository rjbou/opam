...
module Env = struct
  ...
  *)
  ...
  let escape_windows_command_line s =
    ...
    List.fold_left
      (fun acc ch ->
         Re.(replace_string (compile (char ch))
               ~by:("^" ^ String.make 1 ch) acc))
      s ['^'; '"'; '&'; '|'; '<'; '>']
end
...
module OpamSys = struct
  ...
  let windows_ancestor_process_names () =
    let rec helper pid depth =
      if depth > windows_max_ancestor_depth then [] else
      try
  ...
  let guess_dot_profile shell =
    let win_my_powershell f =
      ...
      if Sys.file_exists p then
        Filename.concat (Filename.concat p "PowerShell") f
      else
      let p = Filename.concat (home ()) "My Documents" in
      if Sys.file_exists p then
        Filename.concat (Filename.concat p "PowerShell") f
      else f
    in
    ...
    | SH_pwsh | SH_win_powershell ->
      if Sys.win32 then
        win_my_powershell "Microsoft.Powershell_profile.ps1"
      else

...
let get_cache_cont : type s . s cache_name -> s = function
  ...
  | OpamBS ->
      fun f version key_prefix -> f
        {name = "opam bootstrap";
         key = Fun.const (Printf.sprintf "opam%s-${{ runner.os }}-${{ env.OPAMBSVERSION }}-%s-${{ env.OPAM_REPO_SHA }}-${{ hashFiles ('.github/scripts/main/opam-bs-cache.sh', '.github/scripts/main/preamble.sh') }}" key_prefix version); (* XXX hashFiles shouldn't be here!! *)
         id = "opam-bootstrap";
         force_gzip = false;

...
while read name prefix version url; do
  ...
  versions=$(opam show $package -f all-versions | sed -e 's/  base//' | sed -e 's/  / /g')
  cont=0
  while [ $cont -eq 0 ]; do
    latest=${versions##* }
    if $(opam list --resolve=ocaml.$MIN_OCAML,$package.$latest --quiet > /dev/null 2> /dev/null ); then
      cont=1
    else
      versions=${versions% *}
    fi

...
) else ( if "%GITHUB_ACTIONS%" equ true (
  set BUILD_FOLDER="%GITHUB_WORKSPACE%"
) else (
  set BUILD_FOLDER="%CD%"
))
...
if "%OCAML_PORT%" equ "" "%CYG_ROOT%\bin\bash.exe" -lc "make -C $BUILD_FOLDER run-windows-test" || exit /b 1
goto :EOF

...
## opam-core
  ...
  * `OpamFilename`: add `to_list` that deconstructs a `filename` path [#XXX @rjbou]
  * `OpamFilename.Dir`: add `of_list` that constructs a `dirname` from a string path elements [#XXX @rjbou]
